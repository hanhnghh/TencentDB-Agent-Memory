...
export interface OpenAIMemoryRuntimeAccess {
  userKey: string;
}

export interface OpenAIMemoryRuntimeProvider {
  forRequest(access: OpenAIMemoryRuntimeAccess): MemoryRuntimeContract;
  health?(): Promise<OutboxHealth>;
}

export interface ManagedOpenAIMemoryRuntime {
  provider: OpenAIMemoryRuntimeProvider;
...
export async function createOpenAIMemoryRuntime(
  config: ProxyConfig,
): Promise<ManagedOpenAIMemoryRuntime> {
  ...
  const provider: OpenAIMemoryRuntimeProvider = {
