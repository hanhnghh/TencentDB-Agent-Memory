...
export interface ProxyMemoryRuntimeAccess {
  userKey: string;
}

export interface ProxyMemoryRuntimeProvider {
  forRequest(access: ProxyMemoryRuntimeAccess): MemoryRuntimeContract;
  health?(): Promise<OutboxHealth>;
}

export interface ManagedProxyMemoryRuntime {
  provider: ProxyMemoryRuntimeProvider;
...
export async function createProxyMemoryRuntime(
  config: ProxyConfig,
): Promise<ManagedProxyMemoryRuntime> {
  ...
  const provider: ProxyMemoryRuntimeProvider = {
